...
TEST_P(SequencedRequestTest, ResponseValidation)
{
    ...
    while (true)
    {
        ...
        if (testJson.contains("rpcType"))
        {
            ...
            ASSERT_TRUE(
                SpectreRpcType(
                    responseFull["response"]["type"].get<std::string>()) ==
                reqType.GetResponseType());
            ASSERT_TRUE(responseFull["response"].contains("payload"));
            ASSERT_TRUE(JsonMatchesSchema(responseFull["response"]["payload"],
                                          testJson["responsePayload"],
                                          testJson.contains("ignoreReplace") &&
                                              testJson["ignoreReplace"] == true,
                                          !testJson.contains("ignoreAdd") ||
                                              testJson["ignoreAdd"] == true));
        }
        else
        {
            // http request
            ...
            {
                res = HTTPFetch(8081, testJson["path"], testJson["request"],
            ...
            json expectedResponse =
                json::parse(testJson["response"].get<std::string>());
