...
void SetupLogger()
{
    ...
    logger =
        std::make_shared<spdlog::logger>("pragma", sinks.begin(), sinks.end());
...
void session(tcp::socket sock)
{
    try
    {
        ...
        if (websocket::is_upgrade(req))
        {
            ...
            logger->info(
                "upgraded connection with {}:{} to websocket",
                ws.next_layer().remote_endpoint().address().to_string(),
                ws.next_layer().remote_endpoint().port());

            // basic echo loop so clients have something to talk to
            for (;;)
            {
                beast::flat_buffer wsbuf;
                ws.read(wsbuf); // this blocks until a message arrives or the
                                // peer closes
                SpectreWebsocketRequest req(sock, wsbuf);
                auto route = WebsocketPacketProcessor::GetProcessorForRpc(
                    req.GetRequestType());
                if (route == nullptr)
                {
                    logger->warn(
                        "no packet processor found for WS requestType: " +
        ...
        auto target = stripQueryParams(
            std::string(req.target())); // remove ?query so routing is stable
        HTTPPacketProcessor *processor =
            HTTPPacketProcessor::GetProcessorForRoute(target);
...
void ConnectionAcceptor(unsigned short port)
{
    ...
    {
        asio::io_context
            ioc; // we use sync ops but asio still wants an io_context around

        // tls setup. load cert / key

        tcp::acceptor acc(
            ioc, tcp::endpoint(asio::ip::make_address("0.0.0.0"), port));
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        new HeartbeatProcessor(
            SpectreRpcType("PlayerSessionRpc.HeartbeatV1Request"));
        ...
        new UpdateItemsV0Processor(
            SpectreRpcType("InventoryRpc.UpdateItemsV0Request"));
        new UpdateItemV4Processor(
            SpectreRpcType("InventoryRpc.UpdateItemV4Request"));
        new FieldFetchProcessor<OutfitLoadouts>(
            SpectreRpcType(
                "MtnLoadoutServiceRpc.FetchPlayerOutfitLoadoutsV1Request"),
            FieldKey::PLAYER_OUTFIT_LOADOUT);
        new FetchPlayerLoadoutsProcessor(SpectreRpcType(
            "MtnLoadoutServiceRpc.FetchPlayerWeaponLoadoutsV1Request"));
        ...
        new GetPlayerDataProcessor(SpectreRpcType(
            "MtnPlayerDataServiceRpc.GetAllPlayerDataClientV1Request"));
        new GetBulkProfileDataProcessor(SpectreRpcType(
            "MtnPlayerDataServiceRpc.GetBulkProfileDataClientV1Request"));
        new SavePlayerDataProcessor(SpectreRpcType(
            "MtnPlayerDataServiceRpc.SavePlayerDataForClientV1Request"));
        new FieldFetchProcessor<LegacyPlayerData>(
            SpectreRpcType(
                "MtnPlayerDataServiceRpc.GetPlayerLegacyDataV1Request"),
            FieldKey::PLAYER_LEGACY_DATA);
        new GetLoginDataProcessor(
            SpectreRpcType("GameDataRpc.GetLoginDataV3Request"));
        new CreatePartyProcessor(SpectreRpcType("PartyRpc.CreateV1Request"));
