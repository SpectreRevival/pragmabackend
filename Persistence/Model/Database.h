...
// Template implementations have to be in the headers to not get linker errors,
// :C it's a bit ugly
/* Sample code:
    ...
    std::unique_ptr<PlayerName> nameFetched =
   playerData.GetField<PlayerName>(fetchStatement,
   FieldKey::PLAYER_INGAME_NAME); spdlog::info("name: {}", nameFetched->name());
*/
class Database
{
  private:
    ...
    static std::unordered_map<FieldKey, std::unique_ptr<const pbuf::Message>>
                                             defaultFieldValues;
    template <typename T> std::unique_ptr<T> DefaultOrNullptr(FieldKey key)
    {
        static_assert(std::is_base_of_v<pbuf::Message, T>,
                      "Type provided to DefaultOrNullptr must inherit from "
                      "protobuf::Message");
        ...
        spdlog::info("Returning default value for FieldKey: {}",
  ...
  public:
    Database(const fs::path &dbPath, const std::string &tableName,
             const std::string &keyFieldName, const std::string &keyFieldType);

    sql::Database *GetRaw();
    sql::Database &GetRawRef();

    template <typename T>
    std::vector<std::unique_ptr<T>> GetFields(sql::Statement &query,
                                              FieldKey        key)
    {
        static_assert(
            std::is_base_of_v<pbuf::Message, T>,
            "Type provided to GetFields must inherit from protobuf::Message");
        ...
        while (query.executeStep())
        {
            ...
            {
                spdlog::warn(
                    "Multiple columns returned by query passed into GetFields "
                    "w FieldKey {}, ignoring all columns except first one",
                    classNames.at(key));
            }
            const char *blob =
                static_cast<const char *>(query.getColumn(0).getBlob());
            int sz = query.getColumn(0).getBytes();
            if (sz < sizeof(FieldKey))
            {
                spdlog::error(
                    "sz < {} in GetFields, cell cannot contain a FieldKey",
                    sizeof(FieldKey));
                throw;
            }
            ...
            {
                spdlog::error("FieldKey passed to GetFields {} was not the "
                              "same as FieldKey found in saved object",
                              static_cast<uint32_t>(key));
                throw;
            }
            std::unique_ptr<T> object = std::make_unique<T>();
            if (!object->ParseFromArray(blob + sizeof(FieldKey),
                                        sz - sizeof(FieldKey)))
            {
    ...
    std::unique_ptr<T> GetField(sql::Statement &query, FieldKey key)
    {
        static_assert(
            std::is_base_of_v<pbuf::Message, T>,
            "Type provided to GetField must inherit from protobuf::Message");
        ...
        {
            spdlog::warn(
                "Multiple columns returned by query passed into GetField w "
                "FieldKey {}, ignoring all columns except first one",
                classNames.at(key));
        }
        const char *blob =
            static_cast<const char *>(query.getColumn(0).getBlob());
        ...
        {
            spdlog::error(
                "sz < {} but > 0 in GetField, cell cannot contain a FieldKey",
                sizeof(FieldKey));
            return nullptr;
        }
        FieldKey savedFieldKey;
        ...
        if (savedFieldKey != key)
        {
            spdlog::error("FieldKey passed to GetFields {} was not the same as "
                          "FieldKey found in saved object",
                          static_cast<uint32_t>(key));
            return std::move(DefaultOrNullptr<T>(key));
        }
        std::unique_ptr<T> object = std::make_unique<T>();
    ...
    {
        sql::Statement query =
            FormatStatement("SELECT {col} FROM {table} WHERE " +
                                GetKeyFieldName() + " = ? COLLATE NOCASE",
                            key);
        query.bind(1, dbKey);
    ...
    template <typename T> void AddPrototype(FieldKey key)
    {
        static_assert(std::is_base_of_v<pbuf::Message, T>,
                      "Type provided to AddPrototype must inherit from "
                      "protobuf::Message");
        classNames.insert({key, std::string(T::descriptor()->name())});
        sql::Statement colQuery(m_dbRaw,
        ...
        if (!colExists)
        {
            m_dbRaw.exec("ALTER TABLE " + GetTableName() + " ADD COLUMN " +
                         std::string(T::descriptor()->name()) + " BLOB;");
        }
    }
    ...
    void AddPrototype(const FieldKey key, const fs::path &defaultFieldValuePath)
    {
        ...
        T                 defaultFieldData;
        if (auto status = pbuf::util::JsonStringToMessage(
                data, &defaultFieldData, m_parseOpts);
            !status.ok())
        {
    ...
    static void SetField(sql::Statement &statement, FieldKey key,
                         const pbuf::Message *object, uint32_t dataBindIndex);
    void        SetField(FieldKey key, const pbuf::Message *object,
                         const std::string &ddbKey);
