...
std::unordered_map<FieldKey, std::unique_ptr<const pbuf::Message>>
    Database::defaultFieldValues;

Database::Database(const fs::path &dbPath, const std::string &tableName,
                   const std::string &keyFieldName,
                   const std::string &keyFieldType)
    : m_filename(dbPath),
      m_dbRaw(dbPath.string(), sql::OPEN_READWRITE | sql::OPEN_CREATE),
      m_tableName(tableName), m_keyFieldName(keyFieldName)
{
    m_dbRaw.exec("CREATE TABLE IF NOT EXISTS " + GetTableName() + " (" +
                 GetKeyFieldName() + " " + GetKeyFieldType() +
                 " PRIMARY KEY);");
}
...
* - statement has 1 questionmark, and it's index(1-indexed) is passed for
dataBindIndex Eg valid: INSERT INTO players (PlayerName) VALUES (?) WHERE HI=?
B=2
*/
void Database::SetField(sql::Statement &statement, FieldKey key,
                        const pbuf::Message *object,
                        const uint32_t       dataBindIndex)
{
    if (dataBindIndex == 0)
    {
    ...
    try
    {
        ...
        object->SerializeToArray(bytes.data() + sizeof(FieldKey),
                                 object->ByteSizeLong());
    ...
    {
        spdlog::error("Failed to set field with FieldKey {}: {}",
                      classNames.at(key), e.what());
        throw;
    }
}

void Database::SetField(FieldKey key, const pbuf::Message *object,
                        const std::string &dbKeyId)
{
    sql::Statement setStatement = FormatStatement(
        "INSERT INTO {table} (" + GetKeyFieldName() +
            ", {col}) VALUES(?,?) ON CONFLICT(" + GetKeyFieldName() +
...
{
    sql::Statement query =
        FormatStatement("SELECT {col} FROM {table} WHERE " + GetKeyFieldName() +
                            " = ? COLLATE NOCASE",
                        key);
    query.bind(1, dbKey);
