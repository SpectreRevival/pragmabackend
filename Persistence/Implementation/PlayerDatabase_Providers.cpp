...
static void CheckProviderTable(SQLite::Database &db)
{
    ...
    db.exec("CREATE INDEX IF NOT EXISTS providers_player_idx ON "
            "providers(player_id);");
}

std::string
PlayerDatabase::LookupPlayerByProvider(const std::string &provider,
                                       const std::string &providerId)
{
    ...
    SQLite::Statement query(
        *raw,
        "SELECT player_id FROM providers WHERE provider=? AND provider_id=?;");
...
std::string PlayerDatabase::GetProviderIdByPlayerId(const std::string &playerId,
                                                    const std::string &provider)
{
    auto             *raw = GetRaw();
    SQLite::Statement query(
        *raw,
        "SELECT provider_id FROM providers WHERE player_id=? AND provider=?;");
...
void PlayerDatabase::UpsertProviderMap(const std::string &provider,
                                       const std::string &providerId,
                                       const std::string &playerId)
{
    auto *raw = GetRaw();
    CheckProviderTable(*raw);
    SQLite::Statement query(*raw, "INSERT OR REPLACE INTO providers(provider, "
                                  "provider_id, player_id) VALUES(?,?,?);");
    query.bind(1, provider);
    query.bind(2, providerId);
...
static void CheckBansSchema(SQLite::Database &db)
{
    ...
    SQLite::Statement probe(
        db,
        "SELECT 1 FROM pragma_table_info('bans') WHERE name='banned_until';");
...
bool PlayerDatabase::IsBanned(const std::string &playerId)
{
    auto *raw = GetRaw();
    CheckBansSchema(*raw);
    SQLite::Statement query(*raw,
                            "SELECT banned_until FROM bans WHERE player_id=?;");
    query.bind(1, playerId);
    if (!query.executeStep())
        return false;
