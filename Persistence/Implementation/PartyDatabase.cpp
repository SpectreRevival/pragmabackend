...
{
    sql::Statement colQuery(GetRawRef(),
                            "PRAGMA table_info(" + GetTableName() + ");");
    bool           colExists = false;
    while (colQuery.executeStep())
    {
        if (std::string colName = colQuery.getColumn(1).getText();
            colName == "PartyCode")
        {
            colExists = true;
            break;
    ...
    if (!colExists)
    {
        GetRaw()->exec("ALTER TABLE " + GetTableName() +
                       " ADD COLUMN PartyCode TEXT;");
    }
    AddPrototype<BroadcastPartyExtraInfo>(FieldKey::PARTY_EXTRA_BROADCAST_INFO);
    AddPrototype<BroadcastPrivatePartyExtraInfo>(
        FieldKey::PARTY_PRIVATE_EXTRA_BROADCAST_INFO);
...
void PartyDatabase::SaveParty(const Party &party)
{
    ...
    SetField(FieldKey::PARTY_EXTRA_BROADCAST_INFO, &party.extbroadcastparty(),
             party.partyid());
    SetField(FieldKey::PARTY_PRIVATE_EXTRA_BROADCAST_INFO,
             &party.extprivateplayer(), party.partyid());
    sql::Statement setPartyCode(
        GetRawRef(), "INSERT INTO " + GetTableName() + "(" + GetKeyFieldName() +
                         ", PartyCode) VALUES(?,?) ON CONFLICT(" +
                         GetKeyFieldName() +
                         ") DO UPDATE SET PartyCode = excluded.PartyCode;");
    ...
    {
        spdlog::error("failed to set party code when saving party {}",
                      party.partyid());
        throw;
    }
}
...
{
    Party                         party;
    std::unique_ptr<PartyMembers> members =
        GetField<PartyMembers>(FieldKey::PARTY_MEMBERS, partyId);
    if (!members)
    {
    ...
    sql::Statement getInviteCode(GetRawRef(), "SELECT PartyCode FROM " +
                                                  GetTableName() +
                                                  " WHERE PartyID = ?");
...
{
    Party          party;
    sql::Statement getPartyId(GetRawRef(), "SELECT PartyID FROM " +
                                               GetTableName() +
                                               " WHERE PartyCode = ?");
...
PartyResponse
PartyDatabase::GetPartyResByInviteCode(const std::string &inviteCode)
{
    PartyResponse res;
...
std::string PartyDatabase::SerializePartyToString(const PartyResponse &partyRes)
{
    ...
    if (!status.ok())
    {
        spdlog::error("Failed to serialize CreatePartyProcessor response: {}",
                      status.message());
        throw;
    }
    ...
    {
        spdlog::error(
            "did not find sharedClientData property in CreatePartyProcessor "
            "res json, something weird has happened");
        throw;
    }
    pos += sharedClientDataStart.size();
    std::string jsonfinal =
        std::string(jsoninit.begin(), jsoninit.begin() + pos);
    jsonfinal += '\"';
