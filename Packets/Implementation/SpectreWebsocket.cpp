...
SpectreWebsocket::SpectreWebsocket(ws                                     &sock,
                                   const http::request<http::string_body> &req)
    : socket(sock), curSequenceNumber(0)
{
    socket.auto_fragment(false); // ideally we dont want to have to use this but
                                 // we should be fine for now
    const auto bearer = extract_bearer(req);
    const auto pid =
        bearer.empty() ? std::string() : decode_player_id_noverify(bearer);
...
void SpectreWebsocket::SendPacket(const pbuf::Message &payload,
                                  const std::string &resType, int requestId)
{
    // you shan't comment on this cursedness
    std::string finalRes =
        "{\"sequenceNumber\":" + std::to_string(curSequenceNumber) +
    ...
    {
        spdlog::error(
            "Failed to serialize pbuf message to string in SendPacket");
        throw;
    }
...
void SpectreWebsocket::SendPacket(const std::string &resPayload, int requestId,
...
void SpectreWebsocket::SendNotification(
    const std::shared_ptr<json> &notifPayload,
...
void SpectreWebsocket::SendNotification(const std::string    &notifPayload,
                                        const SpectreRpcType &notificationType)
{
    std::string finalPayload =
        "{\"sequenceNumber\":" + std::to_string(curSequenceNumber) +
        ",\"notification\":{\"type\":\"" + notificationType.GetName() +
...
void SpectreWebsocket::SendNotification(const pbuf::Message  &notif,
                                        const SpectreRpcType &notificationType)
{
    std::string finalRes =
        "{\"sequenceNumber\":" + std::to_string(curSequenceNumber) +
