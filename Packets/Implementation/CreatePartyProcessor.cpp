...
std::string CreatePartyProcessor::GetNewInviteCode()
{
    ...
    while (true)
    {
        ...
        sql::Statement query(*PartyDatabase::Get().GetRaw(),
                             "SELECT 1 FROM " +
                                 PartyDatabase::Get().GetTableName() +
                                 " WHERE PartyCode = ? LIMIT 1");
...
void CreatePartyProcessor::Process(SpectreWebsocketRequest &packet,
                                   SpectreWebsocket        &sock)
{
    std::unique_ptr<CreatePartyRequest> req =
        packet.GetPayloadAsMessage<CreatePartyRequest>();
    ...
    PartyMemberPlayerData *partyPlayerDat =
        creatingPlayerExtra->mutable_playerdata();
    std::unique_ptr<PlayerData> playerDat =
        PlayerDatabase::Get().GetField<PlayerData>(FieldKey::PLAYER_DATA,
                                                   sock.GetPlayerId());
    partyPlayerDat->mutable_defenderweaponloadout()->set_playerid(
        sock.GetPlayerId());
    partyPlayerDat->mutable_defenderweaponloadout()->set_loadoutid(
        playerDat->defenderweaponloadoutid());
    partyPlayerDat->mutable_attackerweaponloadout()->set_playerid(
        sock.GetPlayerId());
    partyPlayerDat->mutable_attackerweaponloadout()->set_loadoutid(
        playerDat->attackerweaponloadoutid());
    partyPlayerDat->mutable_matchmakingdata()->CopyFrom(
        playerDat->matchmakingdata());
    partyPlayerDat->mutable_banner()->CopyFrom(playerDat->banner());

    std::unique_ptr<OutfitLoadouts> outfitLoadouts =
        PlayerDatabase::Get().GetField<OutfitLoadouts>(
            FieldKey::PLAYER_OUTFIT_LOADOUT, sock.GetPlayerId());
    ...
    {
        spdlog::error("Could not find selected outfit loadouts for player {}",
                      sock.GetPlayerId());
        throw std::runtime_error("Could not find selected outfit loadouts");
    }
    partyPlayerDat->mutable_attackeroutfitloadout()->CopyFrom(
        *selectedAttackerOutfit);
    partyPlayerDat->mutable_defenderoutfitloadout()->CopyFrom(
        *selectedDefenderOutfit);

    std::unique_ptr<Inventory> invstruct =
        PlayerDatabase::Get().GetField<Inventory>(FieldKey::PLAYER_INVENTORY,
                                                  sock.GetPlayerId());
    const FullInventory &inv = invstruct->full();
    for (int i = 0; i < inv.instanced_size(); i++)
    {
        // TODO make this only actually return the items needed for performance,
        // but for MVP this should be fine
        creatingPlayerExtra->add_limitedinstancedinventory()->CopyFrom(
            inv.instanced(i));
    }

    PartySharedClientData *sharedData =
        creatingPlayerExtra->mutable_sharedclientdata();
    ...
    std::string steamId = PlayerDatabase::Get().GetProviderIdByPlayerId(
        sock.GetPlayerId(), "STEAM");
    if (steamId.empty())
    {
    ...
    sock.SendPacket(PartyDatabase::SerializePartyToString(createdPartyRes),
                    packet.GetRequestId(), packet.GetResponseType());
}