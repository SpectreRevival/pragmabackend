...
static void SendSuccessfulUpdate(SpectreWebsocketRequest &packet,
...
void UpdateItemV4Processor::Process(SpectreWebsocketRequest &packet,
                                    SpectreWebsocket        &sock)
{
    ...
    std::unique_ptr<Inventory> playerI =
        PlayerDatabase::Get().GetField<Inventory>(invQuery,
                                                  FieldKey::PLAYER_INVENTORY);
    std::unique_ptr<UpdateSingleItemMessage> itemUpdate =
        packet.GetPayloadAsMessage<UpdateSingleItemMessage>();
    ...
    if (itemUpdate->has_instanceditemupdate())
    {
        ...
        {
            if (playerInv->instanced(i).instanceid() ==
                itemUpdate->instanceditemupdate().instanceid())
            {
        ...
        if (!curItem)
        {
            spdlog::warn("Couldn't find item with instance id {} in a item "
                         "update request, skipping",
                         itemUpdate->instanceditemupdate().instanceid());
        ...
        if (itemUpdate->instanceditemupdate()
                .ext()
                .has_progressiontrackerupdate())
        {
            curItem->mutable_ext()
                ->mutable_trackedprogression()
                ->set_activeendorsement(itemUpdate->instanceditemupdate()
                                            .ext()
                                            .progressiontrackerupdate()
                                            .newactiveendorsement());
        }
    }
    if (itemUpdate->has_stackeditemupdate())
    {
        ...
        {
            if (playerInv->stackables(i).instanceid() ==
                itemUpdate->stackeditemupdate().instanceid())
            {
    ...
    PlayerDatabase::Get().SetField(FieldKey::PLAYER_INVENTORY, playerI.get(),
                                   sock.GetPlayerId());
    SendSuccessfulUpdate(packet, sock);
}