...
SpectreWebsocketRequest::SpectreWebsocketRequest(SpectreWebsocket &sock,
                                                 reqbuf            req)
    : m_websocket(sock), m_requestbuf(std::move(req))
{
    json reqjson =
        json::parse(static_cast<const char *>(m_requestbuf.data().data()),
                    static_cast<const char *>(m_requestbuf.data().data()) +
                        m_requestbuf.size());
    ...
    {
        spdlog::warn("log type not found for " +
                     (*m_reqjson)["type"].get<std::string>());
    }
...
std::string SpectreWebsocketRequest::GetResponseType() const
{
    ...
    if (resType.size() >= 7 &&
        resType.compare(resType.size() - 7, 7, "Request") == 0)
