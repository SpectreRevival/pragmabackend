...
void UpdatePartyPlayerProcessor::Process(SpectreWebsocketRequest &packet,
                                         SpectreWebsocket        &sock)
{
    std::unique_ptr<UpdatePartyPlayerRequest> req =
        packet.GetPayloadAsMessage<UpdatePartyPlayerRequest>();
    ...
    {
        spdlog::warn("could not find player(id: {}) in party(id: {}) that was "
                     "being updated by UpdatePartyPlayerProcessor");
        return;
    }
    ...
    // ignoring refreshData, preferredRegions, regionPings, sharedClientData for
    // the moment as they don't seem to be important or used for customgame
