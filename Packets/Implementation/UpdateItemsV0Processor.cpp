...
void UpdateItemsV0Processor::Process(SpectreWebsocketRequest &packet,
                                     SpectreWebsocket        &sock)
{
    ...
    std::unique_ptr<UpdatesItemMessage> itemUpdates =
        packet.GetPayloadAsMessage<UpdatesItemMessage>();
    pbuf::util::JsonPrintOptions options;
    options.always_print_fields_with_no_presence = true;
    for (const InstancedItemUpdate &itemUpdate :
         itemUpdates->instanceditemupdates())
    {
