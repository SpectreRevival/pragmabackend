...
void UpdatePartyProcessor::Process(SpectreWebsocketRequest &packet,
                                   SpectreWebsocket        &sock)
{
    std::unique_ptr<UpdatePartyRequest> req =
        packet.GetPayloadAsMessage<UpdatePartyRequest>();
    PartyResponse res   = PartyDatabase::Get().GetPartyRes(req->partyid());
    Party        *party = res.mutable_party();
    BroadcastPartyExtraInfo *broadcastExtra =
        party->mutable_extbroadcastparty();
    ...
    // Probably need to handle removePlayers at some point but I don't have a
    // request example atm so leaving it for now
    ...
    broadcastExtra->set_hasacceptableregion(
        req->requestext().acceptableregions_size() > 0);
    ...
    sock.SendPacket(PartyDatabase::SerializePartyToString(res),
