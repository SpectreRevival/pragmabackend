...
void SaveOutfitLoadoutProcessor::Process(SpectreWebsocketRequest &packet,
                                         SpectreWebsocket        &sock)
{
    std::unique_ptr<OutfitLoadout> loadoutToSave =
        packet.GetPayloadAsMessage<OutfitLoadout>();
    std::unique_ptr<OutfitLoadouts> loadouts =
        PlayerDatabase::Get().GetField<OutfitLoadouts>(
            FieldKey::PLAYER_OUTFIT_LOADOUT, sock.GetPlayerId());
    if (!loadouts)
        loadouts = std::make_unique<OutfitLoadouts>();

    bool dataWritten = false;
    for (int i = 0; i < loadouts->loadouts_size(); i++)
    {
        if (iequals(loadouts->loadouts(i).loadoutid(),
    ...
    if (!dataWritten)
    {
        spdlog::warn("didn't find the outfit loadout the game was trying to "
