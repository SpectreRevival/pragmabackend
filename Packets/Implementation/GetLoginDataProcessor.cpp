...
void GetLoginDataProcessor::Process(SpectreWebsocketRequest &request,
                                    SpectreWebsocket        &sock)
{
    // todo add player data
    std::string loginDataRes =
        "{\"loginData\":{\"ext\":{\"inboxMessages\":[],\"playerData\":";
    ...
    loginDataRes += ",\"noCrew\":-1.0,\"nextCrewAutomationDate\":\"2025-03-"
                    "04T09:00\",\"crewAutomationInProcess\":false,"
                    "\"currentServiceTimestampMillis\":\"" +
                    std::to_string(duration_cast<std::chrono::milliseconds>(
                                       now.time_since_epoch())
                                       .count()) +
                    "\"},\"inventoryData\":{\"issuedLimitedGrantTrackingIds\":["
                    "],\"inventoryContent\":";
    loginDataRes += *GameDataStore::Get().InventoryStore_buf();
    loginDataRes += "}}}";
    sock.SendPacket(loginDataRes, request.GetRequestId(),
