...
{
    const std::string pem = read_all(
        (ResourcesUtilities::GetResourcesFolder() / "pragma_private.pem")
            .string()); // simply just the JWT priv key.
    ...
    std::unique_ptr<EVP_PKEY, void (*)(EVP_PKEY *)> pkey(pkey_raw,
                                                         EVP_PKEY_free);

    EVP_MD_CTX *ctx_raw = EVP_MD_CTX_new();
    if (!ctx_raw)
        throw std::runtime_error("EVP_MD_CTX_new failed");
    std::unique_ptr<EVP_MD_CTX, void (*)(EVP_MD_CTX *)> ctx(ctx_raw,
                                                            EVP_MD_CTX_free);
    ...
    static AuthCfg cfg = []
    {
        ...
        auto    try_path = [&](const char *p)
        {
            if (std::ifstream f(p); f.is_open())
            {
                ...
                if (!j.is_discarded() && j.contains("steamApiKey") &&
                    j["steamApiKey"].is_string())
                {
...
{
    static const auto ns = boost::uuids::string_generator{}(
        "c8a6b6ce-1e7b-49f2-9a4f-0be3d7b7e5a1");
    const auto id = boost::uuids::name_generator_sha1{ns}(steam64);
    return boost::lexical_cast<std::string>(id);
}

void AuthenticateHandler::Process(http::request<http::string_body> const &req,
                                  tcp::socket                            &sock)
{
    ...
    try
    {
        ...
        if (route == "/v1/submitproviderid")
        {
            ...
            if (steamKey.empty())
            {
                return reply(http::status::bad_request,
                             R"({"error":"steamApiKey not configured"})");
            }

            const auto body = json::parse(req.body(), nullptr, false);
            if (body.is_discarded() || !body.contains("providerId") ||
                !body["providerId"].is_string())
            {
                return reply(http::status::bad_request,
                             R"({"error":"providerId required"})");
            }
            ...
            {
                return reply(http::status::bad_request,
                             R"({"error":"invalid steam id"})");
            }

            AuthLatch::Get().Put(
                client_ip(sock), steam64,
                /*latch timer in seconds*/ 120); // 120s for now until i sort
                                                 // the launcher out - astro
        ...
        if (route == "/v1/account/authenticateorcreatev2")
        {
            ...
            if (playerId.empty())
            {
                ...
                {
                    SteamValidator v(steamKey);
                    if (auto info = v.ValidateSteamId(steam64))
            ...
            if (db.IsBanned(playerId))
            {
                return reply(
                    ...
                    R"({"error":"ACCOUNT BANNED. CONTACT ASTROVAL0 ON DISCORD"})");
            }

            auto prof =
                db.GetField<ProfileData>(FieldKey::PROFILE_DATA, playerId);
            const std::string display =
                prof ? prof->displayname().displayname() : "Player";
            const std::string disc =
                prof ? prof->displayname().discriminator() : "0000";
            const std::string socialId = playerId;

            json tokens = {
                {"pragmaGameToken",
                 BuildJwt("GAME", playerId, socialId, display, disc)},
                {"pragmaSocialToken",
                 BuildJwt("SOCIAL", playerId, socialId, display, disc)}};
    ...
    catch (const std::exception &e)
    {
        ...
        return reply(http::status::internal_server_error,
                     R"({"error":"internal server error"})");
    }
}

std::string
AuthenticateHandler::CreatePlayerFromSteam(const std::string &steam64,
                                           const std::string &displayName)
{
    const std::string uuid = PlayerUuidFromSteam64(steam64);

    std::unique_ptr<ProfileData> pd =
    ...
    std::snprintf(disc, 5, "%04d",
                  std::uniform_int_distribution<int>(0, 9999)(rng));
    dn->set_discriminator(disc);
    PlayerDatabase::Get().SetField(FieldKey::PROFILE_DATA, pd.get(), uuid);
    std::unique_ptr<PlayerData> pdata =
        PlayerDatabase::Get().GetField<PlayerData>(FieldKey::PLAYER_DATA, uuid);
    ...
    std::unique_ptr<OutfitLoadouts> outfits =
    ...
    PlayerDatabase::Get().SetField(FieldKey::PLAYER_OUTFIT_LOADOUT,
                                   outfits.get(), uuid);
    std::unique_ptr<WeaponLoadouts> weapons =
    ...
    PlayerDatabase::Get().SetField(FieldKey::PLAYER_WEAPON_LOADOUT,
                                   weapons.get(), uuid);
    return uuid;
}
...
std::string AuthenticateHandler::BuildJwt(const std::string &backendType,
