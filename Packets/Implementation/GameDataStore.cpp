...
GameDataStore GameDataStore::inst((ResourcesUtilities::GetResourcesFolder() /
                                   "payloads" / "ws" / "game" /
                                   "DefaultInventoryStore.json")
                                      .string());
...
static std::string InventoryStoreToPayload(const InventoryContent *invStore)
{
    ...
    if (!status.ok())
    {
        spdlog::error("Failed to serialize InventoryContent to json string: {}",
                      status.message());
        throw;
    }
    size_t curPos  = 0;
    ...
    while (curPos != std::string::npos)
    {
        ...
        jsonstr2 +=
            std::string(jsonstr.begin() + lastPos, jsonstr.begin() + curPos);
        jsonstr2 += '\"';
        while (curChar != '}')
        {
            if (curChar == '\"')
...
GameDataStore::GameDataStore(std::string inventoryStorePath)
{
    ...
    if (!status.ok())
    {
        spdlog::error("failed to initialize InventoryStore: {}",
                      status.message());
        throw;
    }
...
GameDataStore::InventoryStore_mut()
{
    // When the returned pointer goes out of scope, call the
    // RefreshInventoryStoreCache method in the class
    std::unique_lock storeLock(inventoryStoreLock);
    return std::unique_ptr<InventoryContent,
                           std::function<void(InventoryContent *)>>(
        &inventoryStore,
        // Turns class method into static lambda
        std::bind(&GameDataStore::RefreshInventoryStoreCache, this,
                  std::placeholders::_1));
}

std::unique_ptr<InventoryContent, std::function<void(InventoryContent *)>>
GameDataStore::InventoryStore()
{
    std::unique_lock storeLock(inventoryStoreLock);
    return std::unique_ptr<InventoryContent,
                           std::function<void(InventoryContent *)>>(
        &inventoryStore, std::bind(&GameDataStore::UnlockInventoryStore, this,
                                   std::placeholders::_1));
}

std::unique_ptr<std::string, std::function<void(std::string *)>>
GameDataStore::InventoryStore_buf()
{
    std::unique_lock storeLock(inventoryStoreLock);
    return std::unique_ptr<std::string, std::function<void(std::string *)>>(
        &inventoryStore_bufCache,
        std::bind(&GameDataStore::UnlockInventoryStore2, this,
                  std::placeholders::_1));
}
