...
void SaveWeaponLoadoutProcessor::Process(SpectreWebsocketRequest &packet,
                                         SpectreWebsocket        &sock)
{
    std::unique_ptr<SaveWeaponLoadoutMessage> dataToSave =
        packet.GetPayloadAsMessage<SaveWeaponLoadoutMessage>();
    const WeaponLoadout &loadoutToSave = dataToSave->weaponloadoutdata();

    std::unique_ptr<WeaponLoadouts> loadouts =
        PlayerDatabase::Get().GetField<WeaponLoadouts>(
            FieldKey::PLAYER_WEAPON_LOADOUT, sock.GetPlayerId());

    if (!loadouts)
        loadouts = std::make_unique<WeaponLoadouts>();

    bool dataWritten = false;
    for (int i = 0; i < loadouts->weaponloadoutdata_size(); i++)
    {

        if (iequals(loadouts->weaponloadoutdata(i).loadoutid(),
                    loadoutToSave.loadoutid()))
        {

            loadouts->mutable_weaponloadoutdata(i)->CopyFrom(loadoutToSave);
            loadouts->mutable_weaponloadoutdata(i)->set_playerid(
                sock.GetPlayerId());
    ...
    if (!dataWritten)
    {
        spdlog::warn("didn't find the weapon loadout the game was trying to "
                     "edit, added it as a new loadout\nLoadout id: {}",
                     loadoutToSave.loadoutid());
