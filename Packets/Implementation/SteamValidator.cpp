...
std::string SteamValidator::HttpGet(const std::string &host,
...
std::optional<SteamPlayerInfo>
SteamValidator::ValidateSteamId(const std::string &steam64) const
{
    if (m_apiKey.empty())
        return std::nullopt;

    auto body = HttpGet("api.steampowered.com",
                        "/ISteamUser/GetPlayerSummaries/v2/?key=" + m_apiKey +
                            "&steamids=" + steam64 /* + "&format=json"*/);
