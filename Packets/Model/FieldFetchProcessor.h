...
class FieldFetchProcessor : public WebsocketPacketProcessor
{
  ...
  public:
    ...
    {
        sql::Statement query = m_dbRef.FormatStatement(
            "SELECT {col} FROM {table} WHERE PlayerId=? LIMIT 1", field);
    ...
    static_assert(std::is_base_of<pbuf::Message, T>::value,
                  "Type provided to FieldFetchProcessor must inherit from "
                  "protobuf::Message");
};