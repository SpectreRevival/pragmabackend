...
void RegisterStaticHandlerFromFile(std::string filename, SpectreRpcType rpcType)
{
    ...
    resfile.close();
    spdlog::info("registered static WS {} from json file at {}",
                 rpcType.GetName(), filename);
    new StaticResponseProcessorWS(rpcType,
                                  std::make_shared<json>(std::move(res)));
}

void RegisterRegexHandlerFromFiles(SpectreRpcType rpcType,
                                   std::unordered_map<regex, std::string> map)
{
    ...
    spdlog::info("registered static WS {} using regex handler",
                 rpcType.GetName());
...
void RegisterStaticWSHandlers()
{
             ...
             "ws" / "game"))
    {
        ...
        RegisterStaticHandlerFromFile(fs::absolute(file.path()).string(),
    ...
    RegisterRegexHandlerFromFiles(
        SpectreRpcType("MtnBeaconServiceRpc.GetBeaconEndpointsV1Request"),
        {{regex("hathora-udp\""),
          (ResourcesUtilities::GetResourcesFolder() / "payloads" / "ws" /
           "game" / "beacon" / "hathora-udp.json")
              .string()},
         {regex("hathora\""),
          (ResourcesUtilities::GetResourcesFolder() / "payloads" / "ws" /
           "game" / "beacon" / "hathora.json")
              .string()}});
}
#pragma warning(pop)